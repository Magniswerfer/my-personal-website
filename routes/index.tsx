...
function FeaturedProjectCard({ project }: { project: Project }) {
    return (
        <article class="bg-[#EEE5E9] rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden">
        {project.thumbnail && (
        ...
        <div class="p-6">
        <h3 class="text-xl font-bold text-[#07090F] mb-3">
        <a href={`/projects/${project.slug}`} class="hover:text-[#A5668B]">
        {project.title}
        </a>
        </h3>
        <p class="text-[#7C7C7C] mb-4">
        {project.description}
        </p>
        ...
        {project.technologies.map((tech) => (
            <span 
            key={tech} 
            class="px-3 py-1 bg-white text-[#7C7C7C] rounded-full text-sm"
            >
            {tech}
            </span>
        ))}
        ...
        {project.githubUrl && (
            ...
            class="flex items-center text-[#7C7C7C] hover:text-[#07090F]"
            >
